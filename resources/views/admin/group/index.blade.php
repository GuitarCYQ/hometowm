    ...
    <div class="layui-fluid">
        <div class="layui-row layui-col-space15">
            <div class="layui-col-md12">
                <div class="layui-card">
                    <div class="layui-card-body ">
                        <form class="layui-form-search layui-col-space5">
                            ...
                            <div class="layui-inline layui-show-xs-block">
                                <input class="layui-input"  autocomplete="off" value="{{date("Y-m-d")}}" placeholder="截止日" name="end" id="end">
                            </div>
                            <div class="layui-inline layui-show-xs-block">
                                <input type="text" name="username" id="username_s" placeholder="请输入角色名" autocomplete="off" class="layui-input">
                            </div>
                            <div class="layui-inline layui-show-xs-block">
                    ...
                    <div class="layui-card-header">
                        ...
                        <button class="layui-btn" onclick="xadmin.open('添加用户','{{route('admin.group.create')}}',800,500)"><i class="layui-icon"></i>添加</button>
                    </div>
                    <div class="layui-card-body ">
                ...
                <a title="编辑"  onclick="xadmin.open('编辑','{{route('admin.group.edit',$item['id'])}}')" href="javascript:;">
                                            <i class="layui-icon">&#xe642;</i>
                                        </a>
                                        <a title="删除" val="{{route('admin.group.destroy',$item['id'])}}" class="delbtn" onclick="member_del(this,'{{$item['id']}}')" href="javascript:;">
                                            <i class="layui-icon">&#xe640;</i>
                                        </a>
                                    </td>
                                </tr>
    ...
    <script>
                ...
                $.ajax({
                    ...
                    success:function (data) {
                        if(data.status == 1)
                        {
                            ...
                            $("tbody").html('');
                            var html = '';
                            var editurl = '{{ route('admin.group.edit',$item->id ) }}';
                            for(var i in data.data){
                                // console.log(data.data);
                                html+='<tr><td><input type="checkbox" class="ck" name="delall_id[]"  lay-skin="primary" value="'+data.data[i].id+'"></td><td>'+data.data[i].id+'</td><td>'+data.data[i].group_name+'</td><td>'+data.data[i].group_info+'</td><td class="td-status"><span class="layui-btn layui-btn-normal layui-btn-mini '
        ...
        function member_stop(obj,id){
            layer.confirm('确认要不显示吗？',function(index){
                ...
                if($(obj).attr('title')=='启用'){
                    $.ajax({
                        type:"post",//提交的方式
                        url:"{{route('admin.group.status')}}",//路径
                        dataType:'json',
                        headers: { 'X-CSRF-TOKEN' : '{{ csrf_token() }}' },
                        data : {
                            id:id
        ...
        function delAll (argument) {
            ...
            layer.confirm('确认要删除吗？',function(index){
                ...
                $.ajax({
                    type:"DELETE",//提交的方式
                    url:'{{route('admin.group.delAll',$item['id'])}}',//路径
                    dataType:'json',
                    headers: { 'X-CSRF-TOKEN' : '{{ csrf_token() }}' },
                    data : {
                        id:delall_id
